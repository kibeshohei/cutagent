...
export default function Dashboard({ date }: { date: string }) {
  ...
  const remainKg = +(goal.currentWeight - goal.targetWeight).toFixed(1);
    ...
    <div className="p-4 space-y-4">
      ...
      <div className="bg-white rounded-xl p-4 shadow-sm space-y-2">
        ...
        <p className="text-sm text-gray-500">
          残り {remainKg} kg → {goal.targetWeight} kg（{goal.targetDate}）
        </p>
        {targetDays < 0 && (
          <p className="text-sm text-red-500 font-medium">⚠ 目標日を過ぎています</p>
        )}
      </div>
    </div>
  );
